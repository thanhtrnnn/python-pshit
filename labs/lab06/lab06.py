...
def numberof(thelist, v):
    ...
    if n == 0:
        return 0
    return (thelist[0] == v) + numberof(thelist[1:],v)
...
def number_not(thelist, v):
    ...
    if n == 0:
        return 0
    return (thelist[0] != v) + number_not(thelist[1:], v)
...
def histogram(text):
    ...
    sub_hist = histogram(rest)
    sub_hist[first] = sub_hist.get(first, 0) + 1
    return sub_hist
